...
module.exports = {
    ...
    deleteAddress: async (res, req) => {
        try {
            await Address.findByIdAndDelete(req.params.id);

            res.status(200).json({ status: true, message: "Address succesfully deleted"});
        } catch (error) {
            res.status(500).json({ status: false, message: error.message});
        }
    },
    ...
    setAddressDefault : async (req, res) => {
        ...
        try {
            ...
            if(updatedAddress){
                ...
                res.status(400).json({status: false, message: "Addess not found"});
            }
        } catch (error) {
            res.status(500).json({status: false, message: error.message});
        }
